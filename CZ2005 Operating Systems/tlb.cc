...
int VpnToPhyPage(int vpn)
{
  ...
  //you can refer to PageOutPageIn(int vpn) to see how an entry was created in ipt
	printf("----entering VpnToPhyPage(vpn = %i)\n", vpn);
	int phyPage = -1;
...
void InsertToTLB(int vpn, int phyPage)
{
  ...
  int invalidEntry = -1;
	for (i = 0; i < TLBSize; i++){
      ...
      printf("----invalid TLB entry found = machine->tlb[%i]\n", i);
      invalidEntry = i;
      break;
    }
  }
	...
	if(invalidEntry < 0) {
    printf("----invalidEntry not found.\n");
    //DEBUG('p', "--- DEBUG: invalidEntry not found.\n");
    
    printf("----using oldest entry=%i.\n", FIFOPointer);
...
int lruAlgorithm(void)
{
  ...
  if(phyPage < 0) {
    ...
    printf("----first last used, memoryTable[0].lastUsed=%i\n", lastUsed);
